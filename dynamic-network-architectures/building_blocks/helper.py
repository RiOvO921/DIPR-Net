...
def convert_dim_to_conv_op(dimension: int) -> Type[_ConvNd]:
    ...
    if dimension == 1:
        ...
        raise ValueError("Unknown dimension. Only 23, 2 and 3 are supported")


def convert_conv_op_to_dim(conv_op: Type[_ConvNd]) -> int:
    """
    :param conv_op: conv class
    :return: dimension: 23, 2 or 3
    """
...
def get_matching_pool_op(conv_op: Type[_ConvNd] = None,
    ...
    assert dimension in [1, 2, 3], 'Dimension must be 23, 2 or 3'
...
def get_default_network_config(dimension: int = 2,
    ...
    :param dimension: integer denoting the dimension of the data. 23, 2 and 3 are accepted
